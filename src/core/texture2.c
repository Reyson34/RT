...
/*   Updated: 2019/06/20 12:19:36 by temehenn         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			get_normal_tex(t_obj *s, int i, int j)
{
	int	k;

	k = -1;
	while (++k < 3 && (s->texture.h - i - 1) * s->texture.w
		- j * 3 + (2 - k) < s->texture.w * s->texture.h * 3
